...
export default function Admin() {
    ...
    const predictWaitTime = async () => {
        if (model) {
            ...
            setPrediction(`Predicted number of cars at ${currentTime}: ${predictedCars.toFixed(2)}\nEstimated wait time: ${hours} hours and ${minutes} minutes`);
        } else {
  ...
  onClick={() => { handleItemClick(0); changeUI(); }}
>
  <a className="" href="#">
    <button
...
<li
  className={`${activeIndex === 1 ? 'bg-blue-500 rounded-lg' : ''}`}
  onClick={() => { handleItemClick(1); changeUI2(); }}
>
  <a className="" href="#">
    <button
      className="middle none font-sans font-bold center transition-all disabled:opacity-50 disabled:shadow-none disabled:pointer-events-none text-xs py-3 rounded-lg text-white hover:bg-white/10 active:bg-white/30 w-full flex items-center gap-4 px-4 capitalize"
    ...
    >
      ...
      <p className="block antialiased font-sans text-base leading-relaxed text-inherit font-medium capitalize">
        ...
        <li  className={`${activeIndex === 2 ? 'bg-blue-500 rounded-lg' : ''}`}  onClick={() => { handleItemClick(2); changeUI3(); }} >
          <a class="" href="#">
