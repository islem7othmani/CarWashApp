...
console.log(pwCode)
  ...
  useEffect(() => {
    const verifyActivationCode = async () => {
      try {
        const response = await fetch('http://localhost:8000/authentification/resetPassword', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
          body: JSON.stringify({ resetToken: pwCode }),
        });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      console.log('Submitting postData:', postData); // Debugging line
      ...
      if (!response.ok) {
        ...
        console.error('Response error text:', errorText); // Debugging line
        throw new Error('Network response was not ok');
      }
      ...
      toast.error('An error occurred while resetting your password');  // Show generic error message
    }
  };
          ...
          <div className="mt-5">
            <form onSubmit={handleSubmit}>
              <div className="grid gap-y-4">
                <div>
                  <label htmlFor="newPassword" className="block text-sm font-bold ml-1 mb-2 dark:text-white">New Password</label>
                  ...
                  <p className="hidden text-xs text-red-600 mt-2" id="password-error">Please enter a new password</p>
                </div>
                <button
                  type="submit"
