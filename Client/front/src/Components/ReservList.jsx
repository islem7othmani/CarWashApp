...
const ReservList = () => {
    ...
    useEffect(() => {
        ...
        const fetchReservations = async () => {
            try {
                ...
                reservationsWithUserDetails.sort((a, b) => new Date(a.createdAt) - new Date(b.createdAt));
                ...
                dateTimeMap.forEach((reservationsAtSameTime) => {
                    if (reservationsAtSameTime.length > 1) {
                        reservationsAtSameTime.forEach((reservation) => {
                            if (refusedReservations.has(reservation._id) || checkDuplicate(reservation)) {
                                console.log(`Reservation ID: ${reservation._id}, User ID: ${reservation.user}`);
    ...
    return (
        <div className='absolute left-80'>
            <h2 className='font-semibold text-xl text-blue-600 pb-4'>Reservations for Station</h2>
                            ...
                            {currentReservations.map((reservation, index) => {
                                ...
                                return (
                                    <tr
                                        key={reservation._id}
                                        style={{
                                            backgroundColor: isRefused ? 'lightcoral' : isDuplicate ? 'lightcoral' : isFirstRow ? 'lightgreen' : 'white'
                                        }}
                                    >
                                        <td style={{ border: '1px solid black', padding: '8px' }}>{reservation.userDetails.username || "Unknown"}</td>
                                        <td style={{ border: '1px solid black', padding: '8px' }}>{reservation.carSize}</td>
                                        <td style={{ border: '1px solid black', padding: '8px' }}>{reservation.typeLavage}</td>
                                        <td style={{ border: '1px solid black', padding: '8px' }}>{`${reservation.day}-${reservation.month}-${reservation.year}`}</td>
                                        <td style={{ border: '1px solid black', padding: '8px' }}>{`${reservation.hour}:${reservation.min}`}</td>
                                        <td style={{ border: '1px solid black', padding: '8px' }}>{reservation.stationDetails.nameStation}</td>
                                        <td style={{ border: '1px solid black', padding: '8px' }}>{reservation.stationDetails.city}</td>
                                        <td style={{ border: '1px solid black', padding: '8px' }}>{reservation.stationDetails.state}</td>
                                        <td style={{ border: '1px solid black', padding: '8px' }}>{reservation.stationDetails.CodePostal}</td>
                                        <td style={{ border: '1px solid black', padding: '8px' }} className='space-x-2'>
                                            {!isRefused && (
                                                <>
                    ...
                    <div className="pagination">
                        {Array.from({ length: totalPages }, (_, index) => (
                            <button
                                ...
                                className={`px-4 py-2 border rounded ${currentPage === index + 1 ? 'bg-violet-600 text-white' : 'bg-white text-violet-600'}`}
                            >
                                {index + 1}
                            </button>
                        ))}
